    ...
    for i in (0..node.height-1)
      for j in (0..node.width-1)
        ...
        if flag == 0 #foreground
          ...
          foreground_sum = foreground_sum.component_add(image.raster[node.offset_x+i][node.offset_y+j])
          foreground_counter = foreground_counter + 1
        else
          background_sum = background_sum.component_add(image.raster[node.offset_x+i][node.offset_y+j])
