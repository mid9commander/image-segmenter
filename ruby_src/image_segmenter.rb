# copy from ruby-zbar, need to give proper credit later
...
#    sep_file = make_sep_file(fg_image, bg_image, mask_image)
 ...
 #   puts "sep file created at #{sep_file.path}"
      ...
      for i in (0..node.height-1)
        for j in (0..node.width-1)
          ...
          image.pixel_at(node.offset_x+i, node.offset_y+j) do |pixel|
            ...
            if pixel.class == Array || pixel.class == String#image that use more than 1 byte to represent a pixel


              distance_fg = pixel.inject_with_index(0) {|distance, component, index|
                # puts "component_to_i = #{component.to_i}"
                # puts "component_to_i - fg[index] = #{component.to_i - fg[index]}"
                # puts "distance = #{distance}"
                distance = distance + (component.to_i - fg[index]).abs
                distance
              }
              distance_bg = pixel.inject_with_index(0) {|distance, component, index|
                distance = distance + (component.to_i - bg[index]).abs
                distance
              }

            else
              if pixel.class == Fixnum
                distance_fg = (pixel.to_i - fg).abs
                distance_bg = (pixel.to_i - bg).abs
              end
            end
  ...
  # nodes and just pass down the foreground, background value from the parent to its children
